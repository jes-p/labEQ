...
def invert(ds: lds.LabDataSet, event_num, weights: dict, pre=200, tot_len=2048, extra = 500, dt = 2.5e-8, filt=[1e4,1e5]):
    ...
    traces = ds.get_traces(self, tag, trace_num, event_str=event_str, pre=pre+extra, tot_len=tot_len+2*extra)
    ...
    # TEST HERE: plot the prepped traces on the original #
